...
dashboard via corporate_actions.dashboard_interface.helpers.
"""
import logging
import sys
...
from corporate_actions import config, sources, storage
from corporate_actions.dashboard_interface.helpers import (
    item_from_label,
    label_of,
...
from corporate_actions.telegram.client import NotifierError, is_configured, send_message
...
def persist_watchlist(selected_labels, stock_list):
    ...
    options_keys = {
        (item["exchange"].upper(), item["symbol"].upper()) for item in stock_list
    }
    extras = [
        item for item in saved
        if (item["exchange"].upper(), item["symbol"].upper()) not in options_keys
    ]
...
def show_results_dialog(results: list[dict]):
    ...
    total = len(results)
    new_count = sum(1 for action in results if action.get("new"))
    ...
    if not results:
        ...
        for action in results:
            ...
            if quote and quote.get("price") is not None:
                ...
                change = (f"{'+' if change >= 0 else ''}{change:.2f}%" if change is not None else "-")
            else:
                price, change = None, "-"

            with st.container(border=True):
                st.markdown(f"### {action.get('symbol')} ({action.get('exchange')})")
                st.caption(action.get("company") or " ")
                st.markdown(f"**{action.get('subject')}**")

                metric_col_1, metric_col_2, metric_col_3 = st.columns(3)
                metric_col_1.metric("Ex-Date", action.get("ex_date") or "-")
                metric_col_2.metric("Record Date", action.get("record_date") or "-")
                if price is not None:
                    metric_col_3.metric("Price", f"{price:,.2f}", delta=change)
                else:
                    metric_col_3.metric("Price", "-")

                status = "New" if action.get("new") else "Already sent"
...
with st.sidebar:
    ...
    ui_filters = [
        filter for filter in (ui_settings.get("action_filters") or [])
        if filter in sources.ACTION_TYPES
    ]
    ...
    if (
        ...
        merged["price_alert_pct"] = threshold if threshold > 0 else None
    ...
    with add_col3:
        ...
        if st.button("Add", use_container_width=True):
            ...
            if symbol:
                ...
                if quote is None:
                    st.error(f"Symbol {symbol} not found. Check the ticker and exchange.")
                else:
    ...
    if current_watchlist:
        ...
        if refresh:
            with st.spinner("Fetching prices..."):
                ...
                for item in current_watchlist:
                    ...
                    prices[(item["exchange"], item["symbol"])] = quote
        ...
        for item in current_watchlist:
            ...
            if quote and quote.get("price") is not None:
                ...
                price_txt = f"{quote['price']:.2f} {quote.get('currency', 'INR')}"
            else:
