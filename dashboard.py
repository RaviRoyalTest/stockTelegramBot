...
HELP_TEXT = (
    ...
    "**Market Screens**\n"
    "- `/movers [period] [N] [100|500]` — top movers in a window\n"
    "- `/gainers [period] [N] [100|500]` — top rising stocks\n"
    "- `/losers [period] [N] [100|500]` — top falling stocks\n"
    ...
    "- `/ca [type|symbol|N|today]` — query all actions\n"
    "- `/exdate [today|N]` — ex-date window\n"
    "- `/summary` — counts by exchange & type + next ex-dates\n\n"
    "**Single Stock**\n"
    "- `/stock SYMBOL` — full deep-dive report (valuation, technicals, QoQ holding)\n\n"
    "**Watchlist**\n"
    "- `/add SYMBOL [NSE|BSE]` · `/remove SYMBOL` · `/list` · `/next`\n"
    "- `/news [N|SYMBOL]` — latest headlines\n\n"
    "**Personalise**\n"
    "- `/filter TYPE,TYPE` — receive only chosen action types\n"
    "- `/alert PCT` — alert on ±PCT% daily move\n"
