...
| `/toplosers [period] [N] [100\|500]` | `/toplosers 1mo 100` | Top N losers over an index (same options and fundamentals as `/topgainers`). E.g. `/toplosers 1w 20 500`, `/toplosers 30m 5 nifty100`. Typing bare `/toplosers` shows a rich copy-paste guide - every example is a tappable command you can copy and send (tap the code). Alias: `/losers` |
| `/fundamentalanalyze SYMBOL` | `/fundamentalanalyze TATATECH` | Quick analysis card for one stock: price, 52-week signal, RSI, P/E, sector P/E, market cap, D/E, div yield, current ratio, book value, EPS, net margin, cash & debt, ROCE/ROE, QoQ shareholding. Aliases: `/stock`, `/stockanalysis` |
...
| `/fundamentalreport SYMBOL` | `/fundamentalreport RELIANCE` | DEEP fundamental report for one stock (more detailed than `/fundamentalanalyze`): valuation (P/E, fwd P/E, P/B, P/S, div yield), YoY growth & margins, EPS/book value, balance sheet & cash flow (D/E, current/quick ratio, interest coverage, cash, debt, CFO/CFI/CFF/FCF), analyst targets, QoQ shareholding. Aliases: `/fund`, `/fundamentals` |
| `/fundamentalreport N` / `N-M` / `mylist` | `/fundamentalreport 3-5` | Deep report for a range of watchlist positions (same syntax as `/fundamentalanalyze`, max 5 per query; `mylist` = whole list) |
| `/usstock TICKER` | `/usstock AAPL` | US stock details (live price + deep fundamentals in **USD**): price & change, 52-week range, RSI, P/E, forward P/E, PEG, P/B, P/S, dividend yield, beta, market cap ($B), EV, YoY growth & margins, EPS, book value, cash/debt, D/E, current/quick ratio, free cash flow, ROE/ROCE, analyst targets and employees. Works for any Yahoo US ticker: `/usstock MSFT`, `/usstock NVDA`, `/usstock BRK-B`. Can be scheduled with the US market gate: `/schedule add 3h /usstock AAPL us` (fires only during NASDAQ/NYSE hours). Aliases: `/usfund`, `/usquote`, `/us` |
| `/checklist SYMBOL` / `N-M` / `mylist` | `/checklist RELIANCE` | 32-point investment scorecard (10 personal + 22 AI criteria): D/E, ROE, ROCE, margins, FCF, PEG, P/FCF, EV/EBITDA, P/E vs sector etc. Each item is marked ✅/❌/⚪ (⚪ = needs manual review - no data in the live feed, not counted in the score). `/checklist 5-10` scores watchlist positions #5-#10 (3 per batch). Aliases: `/investcheck`, `/scorecard`, `/qualitycheck` |
...
| Tab | What it does |
| --- | ------------ |
| 📌 **Watchlist** | Load NSE/BSE stock lists, multi-select stocks, add/remove symbols manually (unknown symbols show a "did you mean" pick-list of similar tickers), view live prices, and a ⭐ **My Favourites** bundle (corporate actions for your list, top losers 1h + today, watchlist, deep fundamentals) |
...
| 💹 **Stock Analysis** | **India** (NSE/BSE) quick card **or** DEEP report (like `/fundamentalanalyze` / `/fundamentalreport`) — one symbol or your whole watchlist — **plus 🇺🇸 US stocks** (like `/usstock`): price, 52W signal, RSI, P/E, sector P/E, market cap, D/E, div yield, ROCE, ROE, QoQ shareholding, full fundamentals. Unknown symbols show a pick-list of similar tickers (ticker + full name) |
...
| 🖥️ **System** | Poller status, ⚡ force-check-now button (like `/checknow`), automated-report schedule management with **pause / resume** (like `/schedule pause 1d..1mo`), config, subscribers, persistence / GitHub push status |
...
corporate_actions/
  ...
  market/                   # movement-screen + market-hours helpers (periods,
                            #   change, hours) shared by bot + dashboard
...
- **Where the watchlist lives.** The repo's `watchlist.json` /
  ...
  never push or reset. Always-on hosts like Render have **ephemeral disks** -
