...
def send_message(
    ...
    try:
        ...
        if parse_mode and ("400" in str(error) or "parse" in str(error).lower()):
        ...
        log.warning("Telegram send to chat %s failed: %s", target, config.redact(error))
        ...
        raise NotifierError(config.redact(f"Telegram send failed: {error}")) from error
...
def answer_callback_query(callback_id) -> None:
    ...
    try:
        ...
        log.info("answerCallbackQuery failed: %s", config.redact(error))
...
def set_my_commands(menu: list[dict]) -> bool:
    ...
    try:
        response = requests.post(url, json={"commands": menu}, timeout=config.HTTP_TIMEOUT)
        response.raise_for_status()
        success = bool(response.json().get("ok"))
        log.info("setMyCommands %s", "ok" if success else "failed")
        return success
    except Exception as error:
        log.warning("setMyCommands failed: %s", config.redact(error))
        return False
