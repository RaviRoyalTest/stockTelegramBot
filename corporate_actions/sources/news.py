...
_news_cache: dict = {}
_NEWS_CACHE_SECONDS = 600  # seconds
...
def get_stock_news(exchange: str, symbol: str, limit: int = 3) -> list[dict]:
    ...
    if cached and now - cached["timestamp"] < _NEWS_CACHE_SECONDS:
        return cached["data"]
    ...
    if items:
        _news_cache[cache_key] = {"timestamp": now, "data": items}
    return items
...
def _yf_news(exchange: str, symbol: str, limit: int) -> list[dict]:
    ...
    try:
        ...
        news = (response.json().get("news") or [])[:limit]
        items = []
            ...
            items.append(
                {
                    ...
                    "published_ts": news_item.get("providerPublishTime"),
                }
            )
        return items
    except Exception as error:
        log.info("Yahoo news failed for %s:%s - %s", exchange, symbol, error)
        return []
...
def _google_news(symbol: str, limit: int) -> list[dict]:
    ...
    try:
        ...
        for item in root.iter("item"):
            ...
            timestamp = None
            try:
                timestamp = int(mktime(strptime(published_at, "%a, %d %b %Y %H:%M:%S %Z")))
        ...
        log.info("Google News failed for %s - %s", symbol, error)
        return []
