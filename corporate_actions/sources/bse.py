...
def get_bse_stock_list() -> list[dict]:
    ...
    try:
        response = _session().get(config.BSE_LIST_URL, timeout=config.HTTP_TIMEOUT)
        response.raise_for_status()
        data = response.json()
    except requests.RequestException as error:
        raise SourceError(f"BSE stock list request failed: {error}") from error
    except ValueError as error:
        raise SourceError(f"BSE stock list bad JSON: {error}") from error
...
def get_bse_corporate_actions() -> list[dict]:
    ...
    try:
        response = _session().get(config.BSE_ACTIONS_URL, params=params, timeout=config.HTTP_TIMEOUT)
        response.raise_for_status()
        data = response.json()
    except requests.RequestException as error:
        raise SourceError(f"BSE corporate actions request failed: {error}") from error
    except ValueError as error:
        raise SourceError(f"BSE corporate actions bad JSON: {error}") from error
