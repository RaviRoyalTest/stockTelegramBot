...
COMMAND_LEARN: dict = {
    ...
    "/alertfilters": {
        "what": "The corporate-action alert switch + type filter: silence the automatic dividend/bonus/split/rights/buyback pushes, or keep only the types you choose.",
        "syntax": "/alertfilters off | on | all | TYPE,TYPE",
        "examples": ["/alertfilters off  \u2192 SILENCE all corporate-action alerts", "/alertfilters dividend,bonus  \u2192 only those types", "/alertfilters all  \u2192 all types back on", "/alertfilters on  \u2192 re-enable with your saved types"],
        "output": "Confirmation of the switch + filter; /settings shows it too.",
        "tips": ["On-demand queries (/corpactions, /corpactionsformylist) still work - this only stops the automatic pushes.", "Same switch via /corpactions on|off."],
        "aliases": "/filter, /actionfilters",
    },
    "/pricealert": {
        "what": "Alert when a watched stock moves \u00b1PCT% in a day.",
        "syntax": "/pricealert PCT | on | off",
        "examples": ["/pricealert 3  \u2192 alert on any \u00b13% daily move", "/pricealert off  \u2192 disable (threshold remembered)", "/pricealert on  \u2192 re-enable the last threshold"],
        "output": "Confirmation of the new threshold; the poller then pings you on big moves.",
        "aliases": "/alert",
    },
    "/quiet": {
        "what": "Master pause: mute ALL automatic messages at once - corporate actions, ex-date reminders, price alerts, the watcher and scheduled reports.",
        "syntax": "/quiet on | 2h | off",
        "examples": ["/quiet on  \u2192 pause everything until /quiet off", "/quiet 2h  \u2192 pause for 2 hours (30m, 90, 1d also work)", "/quiet off  \u2192 resume now"],
        "output": "Confirmation with the resume time for timed pauses; /quiet bare shows status.",
        "tips": ["Your commands still work - only background pushes pause.", "A timed pause auto-resumes; /quiet off cancels it early."],
    ...
    "/schedule": {
        ...
        "tips": [
            ...
            "Interval entries (every X) fire exactly on the hour and once more at session close (15:30 IST / 16:00 ET); clock times you choose (at 09:15) fire only at those times.",
...
TOPICS = [
    ...
    },
    {
        ...
        "blurb": "Corporate-action switch, price alerts, the sudden-move watcher, the master /quiet pause and your settings.",
        "commands": ["/alertfilters", "/pricealert", "/watcher", "/quiet", "/fundmode", "/settings"],
    },
    {
        "name": "automation",
...
_TOPIC_ALIASES = {
    ...
    "settings": "alerts", "watcher": "alerts", "quiet": "alerts", "dnd": "alerts",
    "silence": "alerts", "pauseall": "alerts", "automation": "automation", "scheduling": "automation",
...
_COMMAND_ALIASES = {
    ...
    "quiet": "/quiet", "dnd": "/quiet", "silence": "/quiet", "pauseall": "/quiet",
