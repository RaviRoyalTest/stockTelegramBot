...
def render() -> None:
    ...
    ui_filters = [filter for filter in (ui_settings.get("action_filters") or []) if filter in sources.ACTION_TYPES]
    ...
    if (ui_settings.get("action_filters") or []) != sel_types or stored_thresh != threshold:
