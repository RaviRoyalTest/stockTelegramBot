...
def render() -> None:
    ...
    symbol = st.text_input("Symbol (e.g. TATATECH, RELIANCE, INFY)", key="stock_sym").strip().upper()
        ...
        if stock_scope == "My whole watchlist":
            ...
            with st.spinner(f"Fetching analysis for {symbol}..."):
                ...
                if not quote and not fund:
                    ...
                    st.session_state["stock_result"] = {"quote": quote, "fund": fund, "sym": symbol, "deep": deep}
