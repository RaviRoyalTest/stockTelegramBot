...
from ..core.dates import fmt_date, parse_iso_date, today_ist
...
def _price_line(action) -> str | None:
    ...
    return f"Current Price: <b>{fmt_money(price, currency)}</b>"
...
def _offer_window_line(action) -> str | None:
    ...
    if rs and str(rs).strip() not in ("", "-") and re_ and str(re_).strip() not in ("", "-"):
        return f"Offer Window: <b>{fmt_date(rs)}</b> \u2192 <b>{fmt_date(re_)}</b>"
    return None
...
def _book_closure_line(action) -> str | None:
    ...
    if (bc_start and str(bc_start).strip() not in ("", "-")) or (
        ...
        span = " \u2013 ".join(
            fmt_date(d) for d in (bc_start, bc_end)
...
def format_corporate_action(action: dict) -> str:
    ...
    if typ != "other":
        lines.append(f"Type: {TYPE_LABELS.get(typ, typ)}")
    price_line = _price_line(action)
    if price_line:
        lines.append(price_line)
    if ex_date and ex_date != "-":
        lines.append(f"\U0001F4C5 Ex-Date: <b>{fmt_date(ex_date)}</b>")
    if record_date and record_date != "-":
        lines.append(f"Record Date: {fmt_date(record_date)}")
...
def format_upcoming_list(actions: list[dict]) -> str:
    ...
    for action in sorted(actions, key=lambda a: a.get("ex_date") or "9999-99-99"):
        typ = action_type(action.get("subject"))
        lines.append(
            f"\u2022 <b>{escape(action.get('symbol'))}</b> ({escape(action.get('exchange'))}) - "
            f"{escape(action.get('ex_date'))} [{TYPE_LABELS.get(typ, typ)}]"
        )
    return "\n".join(lines)
...
def action_status(action: dict, today=None) -> str:
    ...
    typ = action_type(action.get("subject"))
    ex = parse_iso_date(action.get("ex_date"))
    rec = parse_iso_date(action.get("record_date"))
    ann = parse_iso_date(action.get("announcement_date"))

    if ex is None:
        return "Announced - ex-date not fixed yet (check the company notice for dates)"
    if ex >= today:
        return f"Upcoming - ex-date on {fmt_date(ex)}"

    days_ago = (today - ex).days
    if typ == "rights":
        return (
            f"Ex-date passed {fmt_date(ex)} ({days_ago}d ago) - rights "
    ...
    if typ == "dividend":
        if rec:
            ...
            if today > due:
                return (
                    f"Ex-date passed {fmt_date(ex)} ({days_ago}d ago) - payment "
                    f"window (30 days from record date {fmt_date(rec)}) passed; "
                    "contact broker if not credited"
                )
            return (
                f"Ex-date passed {fmt_date(ex)} ({days_ago}d ago) - payment due "
                f"by {fmt_date(due)} (30 days from record date); contact broker "
                "if not credited"
            )
        return (
            f"Ex-date passed {fmt_date(ex)} ({days_ago}d ago) - payment normally "
            "follows within days-weeks; contact broker if not credited"
        )
    if typ == "bonus":
        return (
            f"Ex-date passed {fmt_date(ex)} ({days_ago}d ago) - bonus shares "
            "usually credited within ~2 weeks of the record date"
        )
    if typ == "split":
        return (
            f"Ex-date passed {fmt_date(ex)} ({days_ago}d ago) - shares "
            "re-denominated to the new face value"
        )
    if typ == "buyback":
        return (
            f"Ex-date passed {fmt_date(ex)} ({days_ago}d ago) - buyback offer "
            "window ongoing; check the offer notice"
        )
    return f"Ex-date passed {fmt_date(ex)} ({days_ago}d ago)"
...
def status_tag(action: dict, today=None) -> tuple[str, str]:
    ...
    if typ == "rights":
        rs, re_ = action.get("rights_start"), action.get("rights_end")
        s = parse_iso_date(rs) if rs else None
        e = parse_iso_date(re_) if re_ else None
        if s and e:
            if today < s:
                return "\U0001F7E1", f"Offer opens {fmt_date(s)}"
            if s <= today <= e:
                return "\U0001F7E2", f"Offer open until {fmt_date(e)}"
            return "\U0001F534", f"Offer closed {fmt_date(e)}"
    ...
    if typ == "dividend":
        if rec:
            ...
            return "\U0001F7E1", f"Payment due by {fmt_date(due)}"
...
def format_action_block(action: dict) -> str:
    ...
    if ann and str(ann).strip() not in ("", "-"):
        lines.append(f"Announced: {fmt_date(ann)}")
    bc = _book_closure_line(action)
    if bc:
        lines.append(bc)
    offer = _offer_window_line(action)
    if offer:
...
def format_action_entry(action: dict) -> str:
    ...
    head += f"  Ex-date: <b>{fmt_date(action.get('ex_date'))}</b>"
    lines = [head]
    bits = []
    if subject != "-":
    ...
    rec = action.get("record_date")
    if rec and str(rec).strip() not in ("", "-"):
        bits.append(f"Record: {fmt_date(rec)}")
    if bits:
...
def format_action_detail(action: dict) -> str:
    ...
    for label, key, fmt in (
        ...
        if val and str(val).strip() and str(val).strip() != "-":
            display = fmt_date(val) if fmt == "date" else escape(val)
