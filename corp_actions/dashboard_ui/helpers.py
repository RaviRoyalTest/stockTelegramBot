...
def style_table(rows: list[dict]) -> "pd.io.formats.style.Styler":
    ...
    def _color_row(row):
        ...
        try:
            ...
            return [f"color:{CELL_NA}"] * len(row)
        color = CELL_UP if up else CELL_DOWN
        styles = [""] * len(row)
...
def fetch_analysis(sym: str) -> dict | None:
...
def fetch_fund_lines(items: list[dict], deep: bool = True) -> list | str:
    ...
    def _one(item):
        ...
        try:
            ...
            fund = sources.get_fundamentals(sym, with_screener=True) or {}
            if deep:
                lines = _fund_report_lines(sym, quote, fund, include_tip=False)
            else:
                lines = _stock_summary_lines(sym, quote, fund, include_tip=False)
            return sym, tg_to_markdown("\n".join(lines))
